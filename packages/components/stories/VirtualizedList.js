...
.virtualized-list div.tc-list-cell-modified { flex: 0 0 90px;}`}
	...
	.add('List > Table : activation', () => (
		<div className="virtualized-list">
			<h1>Virtualized List</h1>
			<p>
				Row double click can be enabled by passing <b>onRowDoubleClick</b> callback. Row click can
				be enabled by passing <b>onRowClick</b> callback and <b>isActive</b> function that returns
				if a row is active.<br />
	...
	.add('List > Large : activation', () => (
		<div>
			<h1>Virtualized List</h1>
			<p>
				Row click can be enabled by passing <b>onRowDoubleClick</b> callback. Row click can be
				enabled by passing <b>onRowClick</b> callback and <b>isActive</b> function that returns if a
				row is active.<br />
				Here example <pre>{'isActive={item => item.id === 6}'}</pre>
			</p>
			<IconsProvider defaultIcons={icons} />
